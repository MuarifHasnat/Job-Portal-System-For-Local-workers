...
$stmt = $conn->prepare("SELECT id, user_id AS customer_id, worker_id, status FROM bookings WHERE id = ?");
...
$body  = "Your booking has been confirmed by the worker.";
...
// Notify worker
$worker_msg = "You successfully confirmed this booking.";
$stmt = $conn->prepare("INSERT INTO notifications (user_id, booking_id, type, title, body, is_read, created_at)
                        VALUES (?, ?, 'booking', ?, ?, 0, NOW())");
$stmt->bind_param('iiss', $worker_id, $booking_id, $title, $worker_msg);
$stmt->execute();

header("Location: /jobportalsystem/bookings/my_bookings.php?confirmed=1");
exit();
?>
